...
func GetRemoteOnlineTable(ctx context.Context) (string, error) {
    ...
    FROM remote 
    LEFT JOIN (SELECT s1.time, s1.tagnumber, s1.domain, s1.broken AS locations_status FROM (SELECT time, tagnumber, domain, broken, ROW_NUMBER() OVER (PARTITION BY tagnumber ORDER BY time DESC) AS row_nums FROM locations) s1 WHERE s1.row_nums = 1) t1
...
func GetRemoteOfflineTable(ctx context.Context) (string, error) {
	...
	sqlCode = `SELECT remote.tagnumber, TO_CHAR(remote.present, 'MM/DD/YY HH12:MI:SS AM') AS time_formatted, 
        remote.status, locations.broken AS locations_status, locationFormatting(locations.location) AS location_formatted, CONCAT(remote.battery_charge, '%', ' - ', remote.battery_status) AS battery_charge_formatted, 
...
func GetTagnumberData(ctx context.Context, tagnumber int) (string, error) {
    ...
    (CASE WHEN locations.broken = TRUE THEN 'Broken' ELSE 'Yes' END) AS remote_status_formatted, locations.broken AS locations_status, t2.department_formatted, t3.note AS most_recent_note,
