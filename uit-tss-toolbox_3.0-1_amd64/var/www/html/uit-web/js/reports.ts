...
type ClientReport = {
	...
	battery_health_variance: number;
...
async function populateBatteryStandardDeviationReport() {
	...
	try {
		...
		data.sort((a: ClientReport, b: ClientReport) => {
			return (b.battery_health_variance || 0) - (a.battery_health_variance || 0);
		});
		...
		for (const report of data) {
			...
			tdBatteryStdDev.textContent = report.battery_health_variance !== undefined ? report.battery_health_variance.toFixed(2) : 'N/A';
