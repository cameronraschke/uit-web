...
async function checkToken(bearerToken = null) {
  return new Promise((resolve, reject) => {
    ...
    fetch('https://UIT_SERVER_WAN_IP_ADDRESS:31411/api/auth?type=check-token', requestOptions)
      .then(response => {
        if (!response.ok) {
  ...
  return new Promise((resolve, reject) => {
    ...
    tokenDB.onsuccess = function(event) {
      ...
      basicTokenRequest.onsuccess = function(event) {
        ...
        fetch('https://UIT_SERVER_WAN_IP_ADDRESS:31411/api/auth?type=new-token', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'credentials': 'include',
